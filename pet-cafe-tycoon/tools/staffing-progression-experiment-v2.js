// Task 24 v2 — deterministic staffing/progression experiment. EXPERIMENT ONLY.
// Rebuilt from the proven economy-bot orchestration after v1 exposed a target-identity reset bug.
import {
...
function nextEssential(s) {
  if ((s.cashier|0) < 1) return 'cashier';
  if ((s.runner|0) < 1) return 'runner';
  if ((s.cleaner|0) < 1) return 'cleaner';
  return null;
}
function mean(rows, fn) { return rows.length ? rows.reduce((a,r)=>a+fn(r),0)/rows.length : 0; }
...
function runScenario(variant, policy, seed) {
  ...
  ensureCareer(G.meta); G.goal=chooseCareerGoal(1,G.meta); world.dayState=G.dayState; world.stars=G.stars;
  ...
  let firstHireAt=null, firstHireKind=null, coreCompleteAt=null;
  ...
  function noteFirstHire(before){
    ...
    firstHireAt=G.time;
    firstHireKind=['cashier','runner','cleaner'].find(k=>(G.staff[k]|0)>(before[k]|0))||'staff';
  }
  function manualHire(kind){
    ...
    purchases[`hire:${kind}`]=(purchases[`hire:${kind}`]||0)+1; world.emit({type:'purchase',kind:`hire:${kind}`,at:G.time}); noteFirstHire(before); return true;
  }
  function maybeCandidateHire(){
    if(!variant.earlyDesk)return false;
    const desk=world.stations.get('hire1'); if(!desk||!desk.active)return false;
    if(totalStaff(G.staff)===0)return manualHire('cashier');
    if(policy.staffFirst){const k=nextEssential(G.staff);if(k)return manualHire(k);}
    return false;
  }
  function reserve(){
    if(!policy.staffFirst)return 0;
    const desk=world.stations.get('hire1'); if(!desk||!desk.active)return 0;
    const k=nextEssential(G.staff); if(!k)return 0;
    const cost=staffingHireCost(variant,k,G.staff); return cost==null?0:Math.min(G.coins,cost);
  }
  ...
  function chooseTarget(){
    ...
    // Candidate first-Cashier price may be below live hireCost. Until manual purchase occurs, keep
    // decide() below the live first-hire threshold so it cannot silently buy at a different price.
    ...
    if(variant.earlyDesk&&policy.staffFirst&&!world.built.has('z_hire')&&target&&(target.kind==='build'||target.kind==='cash')){
      ...
      if(desk&&(G.coins>0||(world.partial.z_hire|0)>0))target={x:desk.x,z:desk.z,kind:'build',zoneId:'z_hire'};
    }
  ...
  return {variant:variant.id,policy:policy.id,seed,firstHireMinutes:firstHireAt==null?null:firstHireAt/60,firstHireKind,entryCost:firstHireEntryCost(variant),coreCompleteMinutes:coreCompleteAt==null?null:coreCompleteAt/60,served,lost,lostRate:lost/Math.max(1,served+lost),goalRate:goals/Math.max(1,dayRows.length),stockoutSeconds,stockoutIncidents,idleSeconds,activeSeconds,finalWallet:Math.round(G.coins),spend:{...spend,total:spend.build+spend.manualHire+spend.decision},purchases,ledgerMismatches,spawnDraws:spawns.snapshot().rngDraws,days:dayRows.length,wallMs:Date.now()-wallStart};
}

function summary(rows){return{runs:rows.length,firstHireMedian:median(rows.map(r=>r.firstHireMinutes)),coreCompleteMedian:median(rows.map(r=>r.coreCompleteMinutes)),lostRate:mean(rows,r=>r.lostRate),goalRate:mean(rows,r=>r.goalRate),stockoutSeconds:mean(rows,r=>r.stockoutSeconds),idleSeconds:mean(rows,r=>r.idleSeconds),finalWallet:mean(rows,r=>r.finalWallet),spend:mean(rows,r=>r.spend.total),served:mean(rows,r=>r.served)};}

export function runStaffingProgressionExperiment(){
  // Mandatory sanity arm: the untouched current configuration must behave like a viable café.
  const sanity=runScenario(CURRENT_STAFFING_VARIANT,STAFFING_POLICIES.balanced,SEEDS[0]);
  if(sanity.served<20||sanity.spend.build<=0||sanity.lostRate>=.8)throw new Error(`Task24 harness baseline invalid: served=${sanity.served}, buildSpend=${sanity.spend.build}, lostRate=${sanity.lostRate.toFixed(3)}`);
  ...
  const eligible=sweep.filter(x=>x.inWindow).sort((a,b)=>a.dist-b.dist||b.variant.firstHireCost-a.variant.firstHireCost);
  const selected=(eligible[0]||sweep.slice().sort((a,b)=>a.dist-b.dist)[0]).variant;
  const runs=[];for(const v of [CURRENT_STAFFING_VARIANT,selected])for(const p of Object.values(STAFFING_POLICIES))for(const seed of SEEDS)runs.push(runScenario(v,p,seed));
  const current=summary(runs.filter(r=>r.variant===CURRENT_STAFFING_VARIANT.id)),candidate=summary(runs.filter(r=>r.variant===selected.id));
  const staffFirstMedian=median(runs.filter(r=>r.variant===selected.id&&r.policy==='staff-first').map(r=>r.firstHireMinutes));
  const lostDelta=candidate.lostRate-current.lostRate,stockoutDelta=candidate.stockoutSeconds-current.stockoutSeconds,completionDelta=(candidate.coreCompleteMedian??99)-(current.coreCompleteMedian??99);
  const allValid=runs.every(r=>!r.ledgerMismatches.length&&r.coreCompleteMinutes!=null&&r.served>0);
  const qualifies=allValid&&staffFirstMedian!=null&&staffFirstMedian>=6&&staffFirstMedian<=10&&lostDelta<=.03&&completionDelta<=4;
  const downside=lostDelta>.005?`lost-sale rate +${(100*lostDelta).toFixed(1)}pp`:stockoutDelta>60?`stockout exposure +${stockoutDelta.toFixed(0)}s/run`:candidate.finalWallet<current.finalWallet?`ending wallet ${Math.round(current.finalWallet-candidate.finalWallet)} lower on average`:'no material measured service downside versus current arm';
  return{sanity,sweep:sweep.map(x=>({variant:x.variant.id,firstHireCost:x.variant.firstHireCost,firstHireMinutes:x.run.firstHireMinutes,coreCompleteMinutes:x.run.coreCompleteMinutes,lostRate:x.run.lostRate,stockoutSeconds:x.run.stockoutSeconds,entryCost:x.run.entryCost,inWindow:x.inWindow})),selected:selected.id,selectedCost:selected.firstHireCost,current,candidate,staffFirstMedian,lostDelta,stockoutDelta,completionDelta,qualifies,downside,runs};
}

if(process.argv[1]&&process.argv[1].endsWith('staffing-progression-experiment-v2.js')){
  const r=runStaffingProgressionExperiment();
  console.log('Pet Café — TASK 24 STAFF-FIRST PROGRESSION EXPERIMENT v2');
  ...
  console.log('--- first-hire sweep: staff-first policy, fixed seed ---');
  for(const x of r.sweep)console.log(`${x.variant.padEnd(17)} entry ${String(x.entryCost.total).padEnd(5)} | hire ${x.firstHireMinutes==null?'never':x.firstHireMinutes.toFixed(1)+'m'} | core ${x.coreCompleteMinutes==null?'n/a':x.coreCompleteMinutes.toFixed(1)+'m'} | lost ${pct(x.lostRate)} | stockout ${x.stockoutSeconds.toFixed(0)}s ${x.inWindow?'TARGET':''}`);
  console.log(`SELECTED FOR ROBUSTNESS: ${r.selected} (first Cashier ${r.selectedCost})`);
  console.log('--- current vs selected: 3 policies × 3 seeds ---');
  for(const v of [CURRENT_STAFFING_VARIANT.id,r.selected])for(const p of Object.values(STAFFING_POLICIES)){const s=summary(r.runs.filter(x=>x.variant===v&&x.policy===p.id));console.log(`${v.padEnd(17)} ${p.id.padEnd(11)} hire ${s.firstHireMedian==null?'never':s.firstHireMedian.toFixed(1)+'m'} | core ${s.coreCompleteMedian==null?'n/a':s.coreCompleteMedian.toFixed(1)+'m'} | served ${s.served.toFixed(0)} | lost ${pct(s.lostRate)} | goals ${pct(s.goalRate)} | stockout ${s.stockoutSeconds.toFixed(0)}s | idle ${s.idleSeconds.toFixed(0)}s | wallet ${Math.round(s.finalWallet)}`);}
  console.log(`CANDIDATE: ${r.qualifies?r.selected:'NONE SUPPORTED YET'}`);console.log(`DOWNSIDE: ${r.downside}`);
  console.log(`COST LEDGER: current entry ${firstHireEntryCost(CURRENT_STAFFING_VARIANT).total}; selected entry ${firstHireEntryCost(STAFFING_CANDIDATES.find(v=>v.id===r.selected)).total}; selected first-hire ${r.selectedCost}`);
  console.log(`ROBUST DELTAS: staff-first hire median ${r.staffFirstMedian?.toFixed(1)??'n/a'}m | lost ${(100*r.lostDelta).toFixed(1)}pp | stockout ${r.stockoutDelta.toFixed(0)}s/run | core completion ${r.completionDelta.toFixed(1)}m`);
  console.log('TASK24_STAFFING_JSON '+JSON.stringify(r));
  let fail=false;for(const x of r.runs){if(x.ledgerMismatches.length){console.error(`${x.variant}/${x.policy}/${x.seed}: ledger mismatch`);fail=true;}if(x.coreCompleteMinutes==null){console.error(`${x.variant}/${x.policy}/${x.seed}: core room did not complete`);fail=true;}}if(fail)process.exitCode=1;
}
