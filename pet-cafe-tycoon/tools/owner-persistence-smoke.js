// Task 11 browser acceptance: the exact YouTube pause-save payload must restore the owner's
// location and each mutually-exclusive hands mode without duplicating or discarding value.
...
async function state() {
  return page.evaluate(() => {
    ...
    return {
      ...
      snapshot: G.snapshot().ownerState,
    };
  });
}
...
try {
  ...
  // Acquire a REAL rendered cookie through the live oven interaction so product-stack restoration
  // proves the renderer and simulation agree, rather than stuffing a fake object into owner.items.
  await page.evaluate(() => {
    const G = window.__game, oven = G.world.stations.get('oven1');
    oven.stock = 12; oven.timer = 0;
    G.P.x = oven.front.x; G.P.z = oven.front.z; G.P.vx = 0; G.P.vz = 0; G._force = null;
  });
  ...
  if (JSON.stringify(beforeProduct.products) !== JSON.stringify(['cookie'])) throw new Error(`live product setup failed: ${JSON.stringify(beforeProduct)}`);
  assertPosition(beforeProduct.position, productPosition, 'pre-save product');
  ...
  if (JSON.stringify(productSave.ownerState.products) !== JSON.stringify(['cookie'])) throw new Error(`product not saved: ${JSON.stringify(productSave.ownerState)}`);
  assertPosition(productSave.ownerState.position, productPosition, 'saved product');
  await page.screenshot({ path:path.join(shots, '01-product-before-reload.png') });

  await page.reload({ waitUntil:'domcontentloaded' });
  await ready();
  await page.waitForTimeout(80);
  const afterProduct = await state();
  if (JSON.stringify(afterProduct.products) !== JSON.stringify(['cookie'])) throw new Error(`product stack not restored: ${JSON.stringify(afterProduct)}`);
  ...
  assertPosition(afterProduct.position, productPosition, 'restored product');
  await page.screenshot({ path:path.join(shots, '02-product-after-reload.png') });

  // Supply sack mode: clear the product stack, retain a partially-used sack, and move elsewhere.
  ...
  await page.screenshot({ path:path.join(shots, '03-sack-after-reload.png') });

  // Fruit mode gets its own round-trip because it uses a separate carry representation/visual.
  ...
  // Strip nested snapshots from the report to keep the artifact concise.
  delete report.product.before.snapshot; delete report.product.after.snapshot; delete report.sack.snapshot; delete report.fruit.snapshot;
