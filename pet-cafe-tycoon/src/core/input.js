...
export function createInput(joyEl, knobEl) {
  ...
  const up = e => { if (e.pointerId !== pid) return; pid = -1; jx = jy = 0; I.pressed = false; joyEl.classList.add('hidden'); };
  ...
  I.dispose = () => { removeEventListener('pointerdown', down); removeEventListener('pointermove', move); removeEventListener('pointerup', up); removeEventListener('pointercancel', up); removeEventListener('keydown', kd); removeEventListener('keyup', ku); removeEventListener('blur', bl); removeEventListener('pointerdown', doFirst); removeEventListener('keydown', doFirst); };
  return I;
}
