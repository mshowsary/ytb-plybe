...
import { iconFor, crateIcon, coinIcon, checkIcon, clockIcon, crossIcon, pawIcon, sparkleIcon } from '../ui/icons.js';
...
function injectStyle() {
  ...
  s.textContent = `
    .party-order-btn{position:fixed;left:calc(12px + env(safe-area-inset-left,0px));top:calc(352px + env(safe-area-inset-top,0px));z-index:15;min-height:44px;min-width:44px;max-width:180px;border:0;border-radius:13px;padding:0 11px;background:#f0ecffed;color:#493b35;box-shadow:0 4px 0 #00000010,0 8px 18px #0000001b;display:flex;align-items:center;gap:7px;font:900 11px/1 system-ui,sans-serif;cursor:pointer}.party-order-box{width:18px;height:18px;flex:none;display:inline-flex}.party-order-box svg{width:100%;height:100%;display:block}.party-order-progress .cueIco{width:15px;height:15px}.party-order-progress{white-space:nowrap}.party-order-ready{color:#6d57d7}.party-order-btn.bump{animation:partyBump .42s ease}@keyframes partyBump{50%{transform:scale(1.12)}}
    ...
    @media(max-width:520px){.party-order-btn{max-width:126px}.party-card{padding:16px}.party-title{font-size:20px}}
    @media(max-width:360px){.party-order-btn{top:calc(350px + env(safe-area-inset-top,0px));min-height:44px;padding:0 9px}.party-foot{align-items:stretch;flex-direction:column}.party-claim{width:100%}}
...
function createUI(onClaim) {
  ...
  // The crate the order physically fills, then the count. "PARTY ORDER" was already hidden below
  // 520px (see the media query above), so on a phone this chip has only ever been a picture and a
  // fraction -- this makes that true at every width.
  btn.innerHTML = `<span class="party-order-box">${crateIcon()}</span><span class="party-order-progress"></span>`;
  ...
  function render(active, day) {
    ...
    // Complete draws the green check the whole game uses for "done"; otherwise the plain fraction.
    btn.querySelector('.party-order-progress').innerHTML = complete ? cueHtml([checkIcon()]) : cueHtml([progress.count, '/', progress.target]);
    btn.setAttribute('aria-label', complete ? `Pet party order ready to claim, ${active.reward} coins` : `Pet party order, ${progress.count} of ${progress.target}`);
...
export function createPartyOrders(G, S, ctx, platform = null) {
  ...
  const claim = () => {
    ...
    hud.setCoins(G.coins); hud.bump(); audio.play('chime'); hud.banner(cue([crateIcon(), coinIcon(), '+', result.reward], `Party order paid, plus ${result.reward} coins`), 2200);
  ...
  collect.style.cssText = 'position:fixed;z-index:16;transform:translate(-50%,-100%);min-height:44px;min-width:44px;border:2px solid #fff7db;border-radius:16px;padding:8px 13px;background:#7654bf;color:white;font:900 13px/1.3 system-ui;box-shadow:0 4px 0 #45316e;cursor:pointer';
  ...
  function updateCollect() {
    ...
    // The drawn coin instead of the emoji one, and no verb: the button sits over a full crate with
    // a payout on it, which is the only thing it could possibly do.
    collect.innerHTML = cueHtml([coinIcon(), active.reward]);
    collect.classList.add('cueRow');
  ...
  function sync(quiet = false) {
    ...
    // Crate, clock, cross: it ran out of time. No coin appears anywhere in the cue, which is the
    // pictogram way of saying "no penalty" -- adding a crossed coin would have implied one.
    if (expired.expired && !quiet) hud.toast(cue([crateIcon(), clockIcon(), crossIcon()], 'Party order expired, no penalty'));
    const start = maybeStartPartyOrder(G.meta, world, day);
    if (start.started && !quiet) { hud.banner(cue([sparkleIcon(), crateIcon(), pawIcon()], 'New pet party order'), 1800); audio.play('ding'); }
  ...
  return {
    ...
    onSale(order) {
      ...
      // The instruction half of this banner ("TAP COLLECT ABOVE THE CRATE") is now redundant as
      // well as wordy: the collect button appears over the crate in the same frame, so the banner
      // only has to say the crate is full.
      if (r.completedNow) { audio.play('chime'); hud.banner(cue([crateIcon(), checkIcon()], 'Party order ready, collect above the crate'), 2600); }
    },
    update(dt) {
      crate.update(dt);
      updateCollect();
