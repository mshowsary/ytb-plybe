...
export function createGuestCare(G,ctx) {
 ...
 button.style.cssText='position:fixed;z-index:17;transform:translate(-50%,-100%);min-height:46px;max-width:170px;border:2px solid #fff;border-radius:15px;padding:10px 12px;background:#487b6b;color:white;font:850 12px/1.3 system-ui;box-shadow:0 4px 0 #294f45;cursor:pointer';document.body.appendChild(button);
 ...
 return {update(dt){
   ...
   if(working){if(blocked||!G.customers.includes(working)||!canReassure(working)||!near(working)){working=null;timer=0;}else{timer+=dt;if(timer>=1.25){if(reassureGuest(working,PATIENCE)){ctx.hud.toast('Reassured · a little more time to serve');ctx.audio.play('ding');G.requestCheckpoint('guest-reassured');}working=null;timer=0;}}}
   ...
   button.textContent=working?'Reassuring… '+Math.round(timer/1.25*100)+'%':'Reassure · +20% patience';
