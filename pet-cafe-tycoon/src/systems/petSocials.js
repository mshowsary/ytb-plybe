...
export function createPetSocials(G,S,ctx) {
  ...
  .social-launch{position:fixed;left:12px;top:350px;z-index:15;border:2px solid #fff7ee;border-radius:16px;background:#426f63;color:white;padding:10px 12px;min-height:46px;font:850 12px/1.3 system-ui;box-shadow:0 4px 0 #25483f;max-width:166px;cursor:pointer}
  ...
  function render() {
    ...
    if(a){
      ...
      if(a.status==='ready') {const b=document.createElement('button');b.className='social-collect';b.textContent=`Collect medal + ${[0,100,200,320][a.tier]} coins`;b.onclick=()=>{const result=claimSocial(state);if(!result)return;G.coins+=result.reward;ctx.hud.setCoins(G.coins);ctx.hud.banner('SOCIAL SUCCESS · '+['','BRONZE','SILVER','GOLD'][result.tier]+' MEDAL',2400);ctx.audio.play('chime');save('pet-social-claim');stamp='';render();};content.appendChild(b);}
      return;
    }
  ...
  return {
    update(){
      ...
      const label=a?(a.status==='ready'?'🏅 Collect social prize':`${SOCIALS.find(s=>s.id===a.id).title}\n${a.count}/${a.target} · ${Math.max(0,Math.ceil(a.deadline-G.dayState.t))}s`):'🐾 Host a Pet Social';
      if(label!==lastLaunch){launch.textContent=label;lastLaunch=label;}render();
    },
    onSale(order){if(recordSocialSale(G.meta.socials,order,G.dayState.day,G.dayState.t)){ctx.hud.banner('PET SOCIAL COMPLETE · COLLECT YOUR MEDAL',2200);ctx.audio.play('chime');save('pet-social-ready');}},
  };
}
