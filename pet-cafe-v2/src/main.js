...
import { PADS, STAFF } from './game/layout.js';
...
let paused = false, hostPaused = false, playTime = 0, saveT = 0, inFlight = 0, completed = false;
...
async function boot() {
  ...
  function grandOpening() {
    hud.banner('🎉 Your bakery is complete!', 3600);
    audio.play('fanfare');
    ...
    setTimeout(() => hud.banner('☕ Coming next: the coffee corner', 3200), 5200);
  }
  function save() { platform.save({ ...W.snapshot(), play: Math.floor(playTime) }); }

  window.__v2 = { W, owner, guests, staff, pads };   // for poking at the live game from the console
  ...
  function frame(now) {
    ...
    W.step(dt);
    staff.update(dt);
    guests.update(dt, owner.o.atTill || staff.cashierAtTill());
    ...
    for (const e of W.events) {
      ...
      } else if (e.type === 'baked') audio.play('drop');
    }
    W.events.length = 0;
...
function guideTarget(owner, staff, guests, playTime) {
  ...
  const c = o.carry;
  if (c.kind === 'flour') { const ov = W.built('oven').find(v => v.flour <= 6); return ov ? { ...ov.spots.work, y: 1 } : { ...W.stations.get('pantry1').spots.work, y: 1 }; }
  if (c.kind) { const ct = W.counterFor(c.kind); return { ...ct.spots.staff, y: 1 }; }
  if (guests.queue.length && !staff.cashierAtTill() && !o.atTill) return { ...W.stations.get('till1').spots.staff, y: 1 };
  for (const ct of W.built('counter')) {
    const ov = W.ovenFor(ct.product);
    const low = playTime < 180 ? ct.cap - 3 : 2;      // new players are shown the bake-and-stock loop straight away
    if (ct.stock <= low && ov.built && ov.tray > 0 && W.freeFor(ct.product) > 0 && !staff.crew.has('runner')) return { ...ov.spots.work, y: 1 };
  }
  const dry = W.built('oven').find(v => v.flour <= 0);
  if (dry && !staff.crew.has('runner')) return { ...W.stations.get('pantry1').spots.work, y: 1 };
  const dirty = W.built('table').find(t => t.dirty);
  if (dirty && !staff.crew.has('cleaner')) return { x: dirty.x, z: dirty.z, y: 1.3 };
  const pad = W.padsOpen().find(p => W.coins + (W.paid[p.id] || 0) >= p.price);
  if (pad) {
    const b = pad.builds;
    const at = b.startsWith('staff:') ? STAFF[b.slice(6)].home : W.stations.get(b);
    return { x: at.x, z: at.z, y: 0.6 };
  }
  return null;
}
